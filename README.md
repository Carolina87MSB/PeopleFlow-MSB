...
**Isso é um pré-cadastro, não um cadastro completo**: `cpf` fica como string vazia (é `NOT NULL` no schema do SST, mas sem CPF real ainda) e `nascimento`/`epis`/`exames` ficam vazios — essas colunas continuam exclusivas do SST, e hoje **não existe tela em nenhum dos dois portais** para completá-las depois da criação (só pelo Supabase Dashboard). Por isso o botão "Novo colaborador" do Portal SST foi removido — ele nunca fez nada (era um `window.alert` de placeholder) e agora a entrada de novo colaborador é sempre pela Admissão aqui no PeopleFlow.

**Não integrado ainda** (fora do escopo desta mudança, mas mapeado): Promoção, Transferência e Mudança de Função concluídas também não atualizam `cargo`/`departamento` em `colaboradores` — a movimentação fica registrada em `peopleflow_movimentacoes`, mas o cadastro-base não reflete a mudança. Alteração Salarial não tem coluna correspondente em `colaboradores` (não guardamos salário ali), então não há o que sincronizar. Desligamento (tipo `DES`) é hoje uma segunda trilha independente do botão "Desligar colaborador" do SST — aprovar a etapa final do `DES` no PeopleFlow **não** marca `desligado = true`.
...
|---|---|---|---|
| `colaboradores` | Portal SST | SST (tudo) e PeopleFlow (nome/cargo/departamento/vinculo/depto_code/nivel/gestor/admissao/desligado/data_desligamento/motivo_desligamento/desligado_by) | Cadastro base: hoje criado só pelo PeopleFlow ao concluir uma Admissão (`api/criar-pre-cadastro.ts`, pré-cadastro sem cpf/nascimento/epis/exames) ou via `npm run seed:supabase` deste projeto (organograma). `desligado`/`data_desligamento`/`motivo_desligamento`/`desligado_by`: só o SST, via `api/desligar-colaborador.ts` dele (service role). `admissao` também editável manualmente pelo RH via `api/atualizar-admissao.ts`. Fora essas exceções pontuais (todas RH-only, via service role), PeopleFlow só lê `colaboradores` |
