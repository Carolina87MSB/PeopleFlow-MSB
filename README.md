...
**Acesso à página**: `/desempenho` é alcançável por qualquer perfil autenticado desde a etapa 2.1 (antes era RH-only — Gestor/Diretoria nunca tinham conseguido chegar lá, nem pelo link do menu nem digitando a URL, o que na prática travava o preenchimento da ficha GESTOR desde a etapa 2). As abas "Configuração"/"Competências comportamentais"/"Competências técnicas"/"Acessos AVD"/"Biblioteca de PDI" continuam RH-only (`podeVerCadastros`); "Avaliações" e "PDI" são abertas a todos — RH vê a seção "Ciclos" (lista administrativa completa + abrir/encerrar ciclo + log de auditoria) e todos os PDIs, Gestor/Diretoria preenchem as fichas dos liderados e veem/editam os PDIs de quem lideram, e o perfil "Colaborador" só enxerga essas duas abas, com suas próprias fichas (e o próprio PDI, só depois de concluído pelo gestor). Preencher/concluir uma ficha específica é liberado pro RH (qualquer uma) ou por quem `gestorAvaliador` aponta como responsável (`podeEditarAvaliacaoDesempenho()` em `usePortalData.ts`) — e só enquanto ela não estiver "Concluída" e o ciclo não estiver "Encerrado". A lista de Avaliações tem filtros por ciclo/status/tipo/departamento; RH também vê a coluna "Avaliador".
