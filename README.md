...
Alguns cargos não precisam do formulário (ex.: "Diretor Geral - CEO", sem descrição formal de cargo) — `precisaDescricaoCargo()` em `src/domain/descricaoCargo.ts` mantém uma lista de exceções (comparação normalizada, sem acento/maiúsculas) e a tela mostra "Não aplicável" no lugar do convite para cadastrar.
