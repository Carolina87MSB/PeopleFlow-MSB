...
### Gestão de Desempenho (`/desempenho`) — etapa 1 (estrutura) + etapa 2 (funcionamento da AVD) + complementação (controle de processo)

Módulo novo, em construção por fases. A etapa 1 entregou a base estrutural (configuração de pesos, catálogo de competências comportamentais, KPIs por cargo). A etapa 2 implementou o **funcionamento de verdade**: RH abre um ciclo de Avaliação de Desempenho, o sistema gera automaticamente a avaliação de cada colaborador ativo, e o gestor preenche notas/resultados com cálculo automático de nota final. A complementação adiciona **controle de processo e rastreabilidade** em cima disso (gerenciamento de ciclos, vínculo automático de gestor/cargo/departamento, autosave, validação de conclusão, auditoria) sem recriar nenhuma regra/cálculo já existente. Ainda **sem** Avaliação de Potencial, Matriz 9 Box, fluxo de aprovação, autoavaliação, calibração, PDI automático, reabertura de ciclo, dashboards ou relatórios gerenciais (fica pra próximas etapas).

8 tabelas (`supabase/schema.sql`, seções 8, 9 e 10), mesmo padrão RLS/comentários do resto do schema:
...
- **`peopleflow_competencias_comportamentais`** — catálogo corporativo, **igual para todos os cargos** (nome, descrição, status ativa/inativa, ordem de exibição e afirmações avaliativas — `afirmacoes jsonb`, cada afirmação é um registro independente da lista, com botões de incluir/excluir/reordenar na ficha de edição, não um campo de texto único). Escala de avaliação (1 a 5, "Não atende" → "Supera as expectativas") é fixa/igual pra todo mundo — vira constante `ESCALA_COMPORTAMENTAL` em `domain/avaliacaoDesempenho.ts`.
...
- **`peopleflow_avaliacoes_desempenho`** — `ciclo_id` referencia o ciclo; `resultados_comportamentais`/`resultados_kpis` guardam o conjunto de competências/KPIs **travado no momento da criação** (não recalculado depois, mesmo que o catálogo mude — evita que uma avaliação em andamento mude de "tamanho" no meio do caminho). Desde a complementação do botão "Abrir ciclo", o **snapshot é completo**: nome/descrição/afirmações da competência e nome/descrição/meta/unidade/sentido/peso do KPI são copiados pra dentro do próprio jsonb no momento da criação (`competenciaNome`/`competenciaDescricao`/`afirmacoes` em cada item de `resultados_comportamentais`; `kpiNome`/`kpiDescricao`/`meta`/`unidadeMedida`/`sentidoMeta`/`peso` em cada item de `resultados_kpis`) — ficam congelados durante todo o ciclo, mesmo que o cadastro original (`competenciasComportamentais`/`kpisCargo`) seja editado ou o item seja desativado/excluído depois. As funções de cálculo (`notaKpi`/`mediaTecnica`/`itensPendentes` em `domain/avaliacaoDesempenho.ts`) usam esse snapshot como fonte primária, com lookup no catálogo atual só como fallback pra avaliações antigas geradas antes desse snapshot existir. `cargo`/`departamento`/`gestor_avaliador` são **snapshot da estrutura organizacional no momento da geração** (mesma lógica, estendida à hierarquia) — preservam o histórico mesmo que o colaborador seja promovido ou troque de gestor depois; colaborador sem gestor cadastrado nasce com `gestor_avaliador` vazio, e essa avaliação só aparece pro RH (nenhum gestor bate com string vazia — RH precisa tratar manualmente; a tela "Ciclos" mostra um badge "N sem gestor" por ciclo, e a lista de Avaliações tem um filtro RH-only "Somente sem gestor"). A visibilidade de cada avaliação pro gestor usa esse `gestor_avaliador` congelado, não o `colaboradores.gestor` atual — se o colaborador trocar de gestor no meio do ciclo, quem avalia continua sendo quem era responsável quando a avaliação foi gerada. `nota_final`/`media_tecnica`/`media_comportamental` são recalculados e regravados a cada save (manual ou automático) — junto com `criado_em`/`concluido_em`, já formam a base do histórico do colaborador (comparativo entre ciclos, evolução, Matriz 9 Box ficam pra etapas futuras, mas o dado já fica disponível: basta filtrar `avaliacoesDesempenho` por `colaboradorNome`). `concluido_por`/`concluido_em` são gravados só na transição pra "Concluída", nunca recalculados depois. **Validação contra duplicidade**: ao gerar as avaliações de um ciclo, `criarCicloComAvaliacoes()` confere no banco quais colaboradores já têm avaliação para aquele `ciclo_id` antes de inserir — protege contra reenvio duplo do formulário/corrida de rede (o id do ciclo é sempre novo por criação, então isso só entra em jogo numa retentativa sobre o mesmo ciclo).
- **`peopleflow_log_avaliacao_desempenho`** *(complementação)* — auditoria básica, append-only: ciclo criado/encerrado, avaliações geradas, avaliação iniciada/salva/concluída (usuário + timestamp). Gravação **best-effort** — nunca lança, só loga no console em caso de falha (`logAvaliacaoDesempenhoRepository.ts`, mesmo espírito do `sst_log` do Portal SST) — nunca bloqueia a ação principal. Exibido como lista crua colapsável dentro da linha de cada ciclo na tela "Ciclos" (RH) — sem dashboard/relatório novo. A data de início de cada avaliação (coluna "Início" na lista de Avaliações) também é derivada daqui (1º registro "AVALIACAO_INICIADA"), sem precisar de um campo dedicado.
...
**Acesso**: abrir/encerrar um ciclo (e ver a seção "Ciclos" com a lista administrativa de todos os ciclos/avaliações/status/avaliador/avaliado/notas e o log de auditoria) é RH-only (`podeVerCadastros`, mesma regra de Departamentos/Cargos/Tipos/Acessos). Preencher/concluir uma avaliação específica é liberado pro RH (qualquer uma) ou pro Gestor cujo colaborador avaliado o tem como `gestor` direto (`podeEditarAvaliacaoDesempenho()` em `usePortalData.ts`, mesmo padrão de `pendenciasAvaliacaoExperiencia`) — e só enquanto a avaliação não estiver "Concluída" e o ciclo não estiver "Encerrado". A lista de Avaliações tem filtros por ciclo/status/departamento; RH também vê a coluna "Avaliador".
