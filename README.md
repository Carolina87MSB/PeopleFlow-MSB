...
- **`peopleflow_competencias_comportamentais`** — catálogo corporativo, **igual para todos os cargos** (nome, descrição, status ativa/inativa, ordem de exibição e afirmações avaliativas — `afirmacoes jsonb`, um array independente por competência, não um campo de texto único). Na ficha de edição (`CompetenciaComportamentalDrawer.tsx`), cada afirmação é uma linha própria, com botões pra incluir, excluir e reordenar (setas cima/baixo) — 3 cadastradas inicialmente por competência, mas o RH pode adicionar mais a qualquer momento. Escala de avaliação (1 a 5, "Não atende" → "Supera as expectativas") é fixa/igual pra todo mundo e por isso não é uma coluna — vira constante de domínio quando o fluxo de avaliação for implementado, mesmo padrão do `ESCALA` hardcoded em `AvaliacaoExperienciaDrawer.tsx`.
