...
### Gestão de Desempenho (`/desempenho`) — etapa 1 (estrutura) + etapa 2 (funcionamento da AVD)

Módulo novo, em construção por fases. A etapa 1 entregou a base estrutural (configuração de pesos, catálogo de competências comportamentais, KPIs por cargo). A etapa 2 implementa o **funcionamento de verdade**: RH abre um ciclo de Avaliação de Desempenho, o sistema gera automaticamente a avaliação de cada colaborador ativo, e o gestor preenche notas/resultados com cálculo automático de nota final. Ainda **sem** Avaliação de Potencial, Matriz 9 Box, fluxo de aprovação, autoavaliação, calibração, reabertura de ciclo, dashboards ou relatórios (fica pra próximas etapas).

7 tabelas (`supabase/schema.sql`, seções 8 e 9), mesmo padrão RLS/comentários do resto do schema:
...
- **`peopleflow_ciclos_avaliacao_desempenho`** *(etapa 2)* — ciclo aberto pelo RH (nome, período de referência, data de início/encerramento). Ao criar, gera automaticamente 1 linha em `peopleflow_avaliacoes_desempenho` por colaborador ativo (elegibilidade nesta etapa: todo colaborador ativo, sem regra de tempo mínimo de empresa ainda).
- **`peopleflow_avaliacoes_desempenho`** *(preenchimento implementado na etapa 2)* — `ciclo_id` referencia o ciclo; `resultados_comportamentais`/`resultados_kpis` guardam o conjunto de competências/KPIs **travado no momento da criação** (não recalculado depois, mesmo que o catálogo mude — evita que uma avaliação em andamento mude de "tamanho" no meio do caminho). Texto de exibição (nome/descrição/afirmações da competência; meta/unidade/sentido/peso do KPI) continua vindo por lookup ao vivo via `competenciaId`/`kpiId` em `competenciasComportamentais`/`kpisCargo` — não duplicado na avaliação.
...
- Status avança sozinho conforme o gestor preenche — "Não iniciada" → "Em andamento" (ao salvar progresso) → "Concluída" (ao concluir, só disponível quando toda afirmação/KPI tem valor) — e **trava a edição por completo** depois de concluída; só uma futura funcionalidade de reabertura de ciclo desfaz isso.
